...
const EMPTY_FORM: FormState = { year_label: "", start_year: "", name: "", notes: "", linkedin_url: "", is_published: true };

function headshotUrl(id: string) {
  const { data } = supabase.storage.from("president_photos").getPublicUrl(`${id}.jpeg`);
...
export function PreviousPresidents() {
  ...
  const [pendingDelete, setPendingDelete] = useState<PresidentRow | null>(null);
  ...
  const openEdit = (row: PresidentRow) => {
    setForm({
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (editing === "new") {
        const created = await insertRow("presidents", values);
        toast.success("President added — you can now upload a photo.");
        setRows((prev) => [...prev, created]);
        setEditing(created);
      } else if (editing) {
        await updateRow("presidents", editing.id, values, editing);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteRow("presidents", pendingDelete.id, pendingDelete);
      await supabase.storage.from("president_photos").remove([`${pendingDelete.id}.jpeg`]);
  ...
  const columns: DataTableColumn<PresidentRow>[] = [
    {
      key: "headshot",
      label: "",
      render: (r) => (
    ...
    { key: "name", label: "Name", render: (r) => r.name, sortValue: (r) => r.name },
    {
      ...
      render: (r) => (
        <PublishToggle
          ...
          onChange={async (next) => {
            try {
              await updateRow("presidents", r.id, { is_published: next }, r);
    ...
    <div className="px-[24px] py-[48px] lg:px-[40px] lg:py-[56px]">
      <div className="flex flex-wrap items-start justify-between gap-[16px]">
        <div>
          ...
          <h1 className="mt-[8px] text-[22px] font-medium text-foreground">Previous Presidents</h1>
        </div>
      ...
      <Drawer open={editing !== null} title={editing === "new" ? "Add president" : "Edit president"} onClose={() => setEditing(null)}>
        <form onSubmit={onSubmit} className="flex flex-col gap-[20px]">
          ...
          <Field label="Notes">
            <textarea rows={3} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="w-full resize-y rounded-[10px] border border-input bg-input px-[14px] py-[12px] text-[15px]! leading-[1.6] text-foreground outline-hidden transition-colors focus:border-accent" />
          </Field>
          ...
          <Field label="Photo">
            {editing === "new" ? (
              ...
              <IdKeyedImageUploader key={photoVersion} bucket="president_photos" id={editing.id} currentUrl={headshotUrl(editing.id)} aspect="square" onUploaded={() => setPhotoVersion((v) => v + 1)} />
            ) : null}
          </Field>
          ...
          <div className="mt-[8px] flex justify-end gap-[8px]">
            ...
            <button type="submit" disabled={saving} className="rounded-[10px] bg-primary px-[16px] py-[10px] text-[13px]! font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-60">
              {saving ? "Saving…" : editing === "new" ? "Add president" : "Save changes"}
            </button>
          </div>
        </form>
      </Drawer>
