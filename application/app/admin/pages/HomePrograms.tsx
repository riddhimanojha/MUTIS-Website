...
type FormState = { title: string; description: string; is_published: boolean };

const EMPTY_FORM: FormState = { title: "", description: "", is_published: true };
...
export function HomePrograms() {
  ...
  const { editing, setEditing, form, setForm, pendingDelete, setPendingDelete, closeDrawer, discardConfirmProps } =
    useDrawerFormCache<ProgramRow, FormState>("homePrograms", EMPTY_FORM);
  ...
  const openEdit = (row: ProgramRow) => {
    setForm({ title: row.title, description: row.description, is_published: row.is_published });
    setEditing(row);
  };
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    try {
      const values = { title: form.title.trim(), description: form.description.trim(), is_published: form.is_published };
      if (editing === "new") {
        const nextOrder = rows.length ? Math.max(...rows.map((r) => r.display_order)) + 1 : 0;
        await insertRow("home_programs", { ...values, display_order: nextOrder });
        toast.success("Program added.");
      } else if (editing) {
        await updateRow("home_programs", editing.id, values, editing);
        toast.success("Program updated.");
      }
      setEditing(null);
      fetchRows();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Could not save that program.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteRow("home_programs", pendingDelete.id, pendingDelete);
      toast.success(`${pendingDelete.title} deleted.`);
      setPendingDelete(null);
      fetchRows();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Could not delete that program.");
    } finally {
      setDeleting(false);
    }
  };
  ...
  const onReorder = async (orderedIds: string[]) => {
    ...
    const updates: { id: string; display_order: number; previous: ProgramRow }[] = [];
    ...
    try {
      await Promise.all(
        updates.map((u) => updateRow("home_programs", u.id, { display_order: u.display_order }, u.previous))
      );
    } catch {
  ...
  const columns: DataTableColumn<ProgramRow>[] = [
    ...
    { key: "description", label: "Description", render: (r) => r.description.slice(0, 80) + (r.description.length > 80 ? "…" : "") },
    {
      key: "is_published",
      label: "Published",
      render: (r) => (
        <PublishToggle
          ...
          onChange={async (next) => {
            try {
              await updateRow("home_programs", r.id, { is_published: next }, r);
    ...
    <div className="px-[24px] py-[48px] lg:px-[40px] lg:py-[56px]">
      <div className="flex flex-wrap items-start justify-between gap-[16px]">
        <div>
          ...
          <h1 className="mt-[8px] text-[22px] font-medium text-foreground">Home "What We Do" Programs</h1>
        </div>
      ...
      <div className="mt-[24px]">
        {loading ? (
          ...
          <ReorderableList
            ...
            onReorder={onReorder}
            renderRow={(r) => (
              <div onClick={() => openEdit(r)} className="grid cursor-pointer grid-cols-[1fr_auto_auto] items-center gap-[12px] py-[10px] pr-[10px] text-[13px]">
          ...
          <DataTable columns={columns} data={filtered} keyField={(r) => r.id} onRowClick={openEdit} emptyMessage="No programs match." />
        )}
      </div>

      <Drawer open={editing !== null} title={editing === "new" ? "Add program" : "Edit program"} onClose={closeDrawer}>
        <form onSubmit={onSubmit} className="flex flex-col gap-[20px]">
          ...
          <Field label="Description" required>
            <textarea rows={4} required value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full resize-y rounded-[10px] border border-input bg-input px-[14px] py-[12px] text-[15px]! leading-[1.6] text-foreground outline-hidden transition-colors focus:border-accent" />
          </Field>
          ...
          <div className="mt-[8px] flex justify-end gap-[8px]">
            ...
            <button type="submit" disabled={saving} className="rounded-[10px] bg-primary px-[16px] py-[10px] text-[13px]! font-medium text-primary-foreground transition-colors hover:bg-primary/90 disabled:opacity-60">
              {saving ? "Saving…" : editing === "new" ? "Add program" : "Save changes"}
            </button>
          </div>
        </form>
      </Drawer>
