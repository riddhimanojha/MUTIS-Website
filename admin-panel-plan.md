...
| 7 | **Spam/rate-limiting hardening** for the three public-insert tables (`contact_submissions`, `sponsorship_enquiries`, `event_signups`) — `BackendPlan.md` already flagged the lack of any hosting-level spam filtering on direct Supabase inserts as a known, accepted gap with a documented future fix (a thin rate-limiting function). Does that hardening need to land before or shortly after this admin panel ships, since the inbox is what will actually surface any spam? |
